...
def run(experiment_dir=None, data_set=None, skipto=None):
    if skipto is None:
        skip_to = STEPS.index("filter")
    else:
        skip_to = STEPS.index(skipto)
    ...
    if STEPS.index("pairwise") >= skip_to:
    ...
    if STEPS.index("cluster") >= skip_to:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skipto', type=str, choices=STEPS, default=STEPS[0],
