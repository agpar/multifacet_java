...
class YelpTrustIndicators:
    ...
    def solo_vector_labels(self):
        return (['user_id'] +
                self.mauro_trust.solo_vector_labels() +
                self.fang_trust.solo_vecto_labels())
    ...
    def single_indicators(self):
        users = list(self._yelp_data.users())
        X = []
        for user in tqdm(users):
            feats = [user['user_id']]
            feats.extend(self.mauro_trust.get_solo_vector(user))
            feats.extend(self.fang_trust.get_solo_vector(user))
            X.append(feats)
        return X
